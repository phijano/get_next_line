...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/03 15:14:59 by phijano-          #+#    #+#             */
/*   Updated: 2022/05/13 14:13:52 by phijano-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd > 4096 || BUFFER_SIZE <= 0)
		return (NULL);
	bytes = BUFFER_SIZE;
	buf_s = ft_get_buf(fd, bytes, buf_s);
	if (!buf_s)
		return (NULL);
	end = ft_endl(buf_s);
	if (end < 0)
		end = ft_strlen(buf_s);
	str = ft_substr(buf_s, 0, end + 1);
	buf_s = ft_get_next_buf(buf_s);
	return (str);
}
